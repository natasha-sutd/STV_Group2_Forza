    ...
    def classify(
        ...
        if "performance bug" in lower or "PerformanceBug" in combined:
            ...
            return self._make_result(
                ...
                raw_key=("performance", "PerformanceBug", perf_msg, stdout_snippet),
                input_data=input_data,
                target=target,
                raw=raw,
            )
        ...
        if "invalidity" in lower or "InvalidityBug" in combined:
            inv_match = re.search(r"(?:InvalidityBug): (.+?)(?:\n|$)", combined)
            inv_msg = inv_match.group(1)[:160] if inv_match else exc_msg
            stdout_snippet = stdout[:80].strip() if stdout else ""
            return self._make_result(
                bug_type=BugType.INVALIDITY,
                raw_key=("invalidity", "InvalidityBug", inv_msg, stdout_snippet),
                input_data=input_data,
                target=target,
                raw=raw,
            )
        ...
        if (
            ...
            return self._make_result(
                ...
                raw_key=("syntactic", "AddrFormatError", syn_msg, stdout_snippet),
                input_data=input_data,
                target=target,
                raw=raw,
            )
        ...
        if "functional" in lower and "functional bug" in lower:
            ...
            return self._make_result(
                ...
                raw_key=("functional", "FunctionalBug", func_msg, stdout_snippet),
                input_data=input_data,
                target=target,
                raw=raw,
            )
